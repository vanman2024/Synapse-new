...
if grep -q "<summary>" "$LOG_FILE" 2>/dev/null; then
  ...
  log_info "Creating compact summary file at $COMPACT_FILE"
  {
    echo "# Claude Compact Summary - $(date +"%B %d, %Y")" > "$COMPACT_FILE"
    echo "" >> "$COMPACT_FILE"
    cat "$LOG_FILE.extract" >> "$COMPACT_FILE"
  } || {
    log_error "Failed to create compact summary file" "Path: $COMPACT_FILE"
    exit 1
  }
  
  # Also create archive version
  log_info "Creating archive text file at $ARCHIVE_FILE"
  {
    echo "Session from $(date +"%B %d, %Y")" > "$ARCHIVE_FILE"
    echo "" >> "$ARCHIVE_FILE"
    cat "$LOG_FILE.extract" >> "$ARCHIVE_FILE"
  } || {
    log_error "Failed to create archive text file" "Path: $ARCHIVE_FILE"
    exit 1
  }
  
  log_success "Saved compact summary to $COMPACT_FILE"
  log_success "Created archive at $ARCHIVE_FILE"
  echo "Archive creation complete" >> "$DEBUG_LOG"
