...
show_help() {
  ...
  echo "and maintains a single source of truth for tracking using Airtable."
  echo ""
  echo -e "${GREEN}Session Management:${NC}"
  echo "  start         - Start a new development session with auto-tracking in Airtable"
  echo "  end           - End the current session and update Airtable"
  echo "  status        - Show current project and session status"
  echo "  cleanup       - Clean up temporary session files"
  echo ""
